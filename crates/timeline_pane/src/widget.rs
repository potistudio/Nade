...
impl std::fmt::Debug for TimelineWidget {
...
impl TimelineWidget {
	...
	/// 状態への参照を取得
	pub fn state(&self) -> std::cell::Ref<'_, TimelineState> {
		self.state.borrow()
	}

	/// 状態への可変参照を取得
	pub fn state_mut(&self) -> std::cell::RefMut<'_, TimelineState> {
...
impl canvas::Program<TimelineMessage> for TimelineProgram {
	type State = ();
...
impl TimelineProgram {
	fn draw_timeline(&self, frame: &mut canvas::Frame, bounds: Size) {
		...
		self.draw_playhead(frame, &state, layout.ruler, layout.content);
	}

	fn draw_ruler(&self, frame: &mut canvas::Frame, state: &TimelineState, rect: Rectangle) {
	...
	fn draw_track_labels(&self, frame: &mut canvas::Frame, state: &TimelineState, rect: Rectangle) {
