...
impl Value {
	...
	pub fn as_f32(self) -> Result<f32, EvalError> {
		match self {
			Self::Float(v) => Ok(v),
			other => Err(EvalError::ExpectedFloat {
				found: other.kind(),
			}),
		}
	}
	...
	pub fn as_vec4(self) -> Result<[f32; 4], EvalError> {
		match self {
			Self::Vec4(v) => Ok(v),
			other => Err(EvalError::ExpectedVec4 {
				found: other.kind(),
			}),
		}
	}
	...
	pub fn add_value(self, other: Value) -> Result<Value, EvalError> {
		match (self, other) {
			...
			(Value::Float(a), Value::Vec4(b)) => {
				Ok(Value::Vec4([a + b[0], a + b[1], a + b[2], a + b[3]]))
			}
			(Value::Vec4(a), Value::Float(b)) => {
				Ok(Value::Vec4([a[0] + b, a[1] + b, a[2] + b, a[3] + b]))
			}
