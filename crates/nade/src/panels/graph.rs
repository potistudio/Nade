use crate::message::{AppPanelMessage, GraphMessage};
use crate::panel_content::PanelContent;
use crate::theme::panel::GRAPH_BG;
...
impl GraphUiState {
	pub fn new() -> Self {
		...
		let (graph, ids) = build_sample_graph(
			freq, phase, blur_base, blur_scale, color_a, color_b, 256, 256,
		);
	...
	pub fn update(&mut self, msg: GraphMessage, time: f64) {
		match msg {
	...
	fn set_value_const(&mut self, node_id: NodeId, value: Value) {
		if let Some(node) = self.graph.node_mut(node_id) {
			if let Some(op) = node.operator.as_any_mut().downcast_mut::<ValueConstOp>() {
				op.value = value;
				self.graph.clear_cache();
			}
		}
...
pub fn view<'a>(
	state: &'a GraphUiState,
	preview_time: f32,
) -> Element<'a, panel_system::PanelSystemMessage<PanelContent, AppPanelMessage>> {
	let header = row![
		text("Resolution Graph").size(14).color(TEXT_PRIMARY),
	...
	let fps_row = row![
		...
		draggable_number(state.eval_fps, 60.0, GraphMessage::EvalFpsChanged).step(1.0)
	]
	.spacing(10)
	.align_y(Alignment::Center);
	...
	let lfo_section = column![
		section_title("LFO"),
		param_row("Freq (Hz)", state.freq, GraphMessage::FreqChanged, 0.01),
		param_row("Phase", state.phase, GraphMessage::PhaseChanged, 0.01),
	...
	let blur_section = column![
		...
		param_row(
			"Base Radius",
			state.blur_base,
			GraphMessage::BlurBaseChanged,
			0.1,
		),
		param_row(
			"Scale",
			state.blur_scale,
			GraphMessage::BlurScaleChanged,
			0.1,
		),
	...
	let color_section = column![
		...
		color_row("Color A", state.color_a, GraphMessage::ColorAChanged),
		color_row("Color B", state.color_b, GraphMessage::ColorBChanged),
	]
	.spacing(6);
	...
	let delay_row = row![
		checkbox(state.use_value_delay)
			.label("Value Delay1")
			.on_toggle(GraphMessage::UseValueDelay),
		checkbox(state.use_image_delay)
			.label("Image Delay1")
			.on_toggle(GraphMessage::UseImageDelay),
	]
	.spacing(10);

	let evaluate_button = button(text("Evaluate").size(12)).on_press(GraphMessage::Evaluate);
	...
	let preview: Element<'_, GraphMessage> = if let Some(frame) = &state.last_frame {
	...
	let panel: Element<'_, GraphMessage> = container(content)
		.width(Length::Fill)
		.height(Length::Fill)
		.style(|_| container::Style {
			background: Some(GRAPH_BG.into()),
			..Default::default()
		})
		.into();

	panel.map(|msg| panel_system::PanelSystemMessage::AppMessage(AppPanelMessage::Graph(msg)))
}
...
fn param_row<'a>(
	label: &'a str,
	value: f32,
	on_change: impl Fn(f32) -> GraphMessage + 'a,
	step: f32,
...
fn value_row<'a>(label: &'a str, value: Option<f32>) -> Element<'a, GraphMessage> {
	let content = value
		.map(|v| format!("{v:.3}"))
...
fn color_row<'a>(
	label: &'a str,
	color: [f32; 4],
	on_change: impl Fn(usize, f32) -> GraphMessage + 'a + Clone,
) -> Element<'a, GraphMessage> {
...
fn divider<'a>() -> Element<'a, GraphMessage> {
...
fn build_sample_graph(
	...
	let freq_node = graph.add_node(ValueConstOp::new(Value::Float(freq)));
	let phase_node = graph.add_node(ValueConstOp::new(Value::Float(phase)));
	...
	let lfo_delay_node =
		graph.add_node(ValueDelay1Op::new(ValueParam::with_mapping(Value::Float(0.0), lfo_node)));

	let blur_base_node = graph.add_node(ValueConstOp::new(Value::Float(blur_base)));
	let blur_scale_node = graph.add_node(ValueConstOp::new(Value::Float(blur_scale)));
	...
	let color_a_node = graph.add_node(ValueConstOp::new(Value::Vec4(color_a)));
	let color_b_node = graph.add_node(ValueConstOp::new(Value::Vec4(color_b)));
