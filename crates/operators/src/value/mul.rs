...
#[derive(Debug, Clone)]
pub struct ValueMulOp {
	pub a: ValueParam,
	pub b: ValueParam,
}

#[derive(Debug, Clone)]
struct ValueMulResolved {
	a: Value,
	b: Value,
}
...
impl Operator for ValueMulOp {
	...
	fn resolve(&self, eval: &mut dyn EvalAccess, time: f64, ctx: &EvalContext) -> Result<ResolvedOp, EvalError> {
		...
		Ok(ResolvedOp::new(ValueMulResolved { a: a_val, b: b_val }, 0, input_hash))
	}
	...
	fn compute(
		...
		let resolved = resolved.take::<ValueMulResolved>()?;
		Ok((Output::Value(resolved.a.mul_value(resolved.b)?), NodeState::Empty))
	}
