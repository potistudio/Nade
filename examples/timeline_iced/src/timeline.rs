...
("Audio", vec![
("BGM", 0.0, 11.0, Color::from_rgb8(234, 67, 53)),
]),
("Effects", vec![
...
track.add_clip(TimelineClip::new(self.next_clip_id(), name, start, duration, color));
}
...
self.draw_vertical_line(frame, x, rect.y + rect.height - tick_height, rect.y + rect.height, tick_color);
...
let bg_color = if i % 2 == 0 { colors::TRACK_LABEL_BG } else { colors::TRACK_LABEL_BG_ALT };
frame.fill_rectangle(Point::new(rect.x, y), Size::new(rect.width, TRACK_HEIGHT), bg_color);

let text_color = if track.muted { colors::TEXT_MUTED } else { colors::TEXT_PRIMARY };
frame.fill_text(Text {
...
self.draw_horizontal_line(frame, rect.x, rect.x + rect.width, y + TRACK_HEIGHT, colors::BORDER_SUBTLE);
}
}
...
let bg_color = if track_index % 2 == 0 { colors::TIMELINE_BG_ALT } else { colors::TIMELINE_BG_ALT2 };
frame.fill_rectangle(Point::new(rect.x, y), Size::new(rect.width, TRACK_HEIGHT), bg_color);
...
self.draw_horizontal_line(frame, rect.x, rect.x + rect.width, y + TRACK_HEIGHT, colors::BORDER_DARK);
}
}
...
let clip_color = if is_selected { lighten_color(clip.color, 0.2) } else { clip.color };
...
frame.stroke(&clip_path, Stroke::default().with_color(colors::CLIP_OUTLINE).with_width(1.0));
...
fn draw_clip_name(&self, frame: &mut canvas::Frame, clip: &TimelineClip, clip_rect: Rectangle, timeline_rect: Rectangle) {
...
fn draw_playhead(&self, frame: &mut canvas::Frame, ruler_rect: Rectangle, timeline_rect: Rectangle) {
...
// タイムライン上の縦線
self.draw_vertical_line_with_width(frame, x, timeline_rect.y, timeline_rect.y + timeline_rect.height, colors::PLAYHEAD, 2.0);
}
...
fn draw_vertical_line_with_width(&self, frame: &mut canvas::Frame, x: f32, y1: f32, y2: f32, color: Color, width: f32) {
let line = Path::line(Point::new(x, y1), Point::new(x, y2));
...
DragState::Clip { track_id, clip_id, offset } => {
...
DragState::ClipResizeLeft { track_id, clip_id, original_start, original_duration } => {
...
DragState::Panning { start_offset, start_cursor } => {
...
state.draw(&mut frame, bounds.size());
...
cursor_position
.map(|pos| state.handle_mouse_press(Point::new(bounds.x + pos.x, bounds.y + pos.y), bounds))
.unwrap_or(false)
}
...
cursor_position
.map(|pos| state.handle_mouse_move(Point::new(bounds.x + pos.x, bounds.y + pos.y), layout.timeline_left()))
.unwrap_or(false)
}
...
cursor
.position_in(bounds)
.map(|pos| state.get_mouse_interaction(Point::new(bounds.x + pos.x, bounds.y + pos.y), bounds))
.unwrap_or_default()
}
}
